...
POINT = 1
POLYLINE = 3 # 4 Bezier
POLYGON = 5 # 6 Bezier Polygon (Area)
MULTIPOINT = 8
POINTZ = 11
POLYLINEZ = 13
POLYGONZ = 15
...
class Reader:
    ...
    def __shape(self, prj=False):
        ...
        if nPoints:
            if prj:
                record.points = [prj.map_to_user(_Array('d', unpack("<2d", f.read(16)))) for p in range(nPoints)]
            else:
        ...
        if shapeType in (1,11,21):
            if prj:
                record.points = prj.map_to_user(_Array('d', unpack("<2d", f.read(16))))
            else:
    ...
    def ogis_shapes(self, prj):
        """Returns all shapes in a shapefile."""
        shp = self.__getFileObj(self.shp)
        shp.seek(100)
        shapes = []
        while shp.tell() < self.shpLength:
            shapes.append(self.__shape(prj))
        return shapes
